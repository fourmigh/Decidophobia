...
object OptionsUtils {
    ...
    fun insert(context: Context, etTitle: EditText, etOption: List<EditText>, size: Int): Boolean {
        val options = createOptions(etTitle, etOption, size)
        val database = OptionsDatabase.getInstance(context)
    ...
    fun query(options: List<Options>?): Array<String> {
        val size = options?.size?:0
        val strings = Array(size) { index -> index.toString() }
        if (options != null) {
    ...
    private fun createOptions(etTitle: EditText, etOption: List<EditText>, size: Int): Options {
        ...
        val option = SerializedList<String>()
        for (i in 0..(size-1)) {
